...
**Q: 计划写太细是不是浪费时间？**
2-5 分钟的粒度恰好——足够清晰到 agent 不偏离，又不至于琐碎。模糊的 plan 等于没写。

**Q: 计划和 spec 有什么区别？**
Spec 说**要做什么**（what），Plan 说**怎么做**（how），精确到文件路径和代码。

**Q: 什么时候不需要写 plan？**
修改单个文件的 bug（用 system debugging + TDD 更快）、纯配置变更、一次性脚本。但任何涉及多文件、多步骤的工作——哪怕"只花 30 分钟"——都应该写 plan。Plan 花 5 分钟写，省 30 分钟返工。

**Q: 自审发现 spec 有问题怎么办？**
比如 spec 的要求在技术上不可能、或者两个要求互相矛盾。这不算 plan 失败——在自审记录里写明发现了什么，然后回到 brainstorming 修改 spec。不要把 spec 的问题藏进 plan 里。**

**Q: 多子系统但 spec 没拆怎么办？**
主动提出拆。一个子系统一个 plan。不要说"spec 就这样了，硬写一个 plan"。
